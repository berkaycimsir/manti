...
} from "~/server/db/connection-state";
...
class DatabaseConnectionPool {
	...
	private maxPoolSize = 50; // Increased limit for server environment
	...
	async getConnection(
		...
		if (this.connections.size >= this.maxPoolSize) {
			...
			let lruConnectionId: number | null = null;
			let oldestTimestamp = Number.POSITIVE_INFINITY;

			for (const [id, entry] of this.connections.entries()) {
				if (!entry.closing && entry.lastUsed < oldestTimestamp) {
		...
		// Restore persisted lastUsed if available
		...
		// LRU uses current time; DB timestamp updated only if requested
