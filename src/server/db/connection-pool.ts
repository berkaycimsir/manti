...
import { markConnectionAsInactive } from "~/server/db/query-utils";
import {
...
class DatabaseConnectionPool {
	...
	private async cleanupIdleConnections(): Promise<void> {
		...
		for (const [id, entry] of this.connections.entries()) {
			...
			console.log(
				`[ConnectionPool] Connection ${id}: idle for ${Math.round(idleTime / 1000)}s (timeout: ${Math.round(this.idleTimeout / 1000)}s)`,
			);
