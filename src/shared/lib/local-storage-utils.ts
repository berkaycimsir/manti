...
export function getTableStoreSummary(
	...
	if (storeKey === "table-density-storage") {
		const density = state.densityModes?.[targetKey];
		return density ? formatDensity(density) : null;
	}
	if (storeKey === "table-view-mode-storage") {
		const viewMode = state.viewModes?.[targetKey];
		return viewMode ? formatViewMode(viewMode) : null;
	}
	if (storeKey === "table-options-storage") {
		...
		return active.length > 0 ? active.join(", ") : "Default options";
	}
	if (storeKey === "text-view-options-storage") {
		const opts = state.options?.[targetKey];
		return opts ? `Char Limit: ${opts.maxCharacters}` : null;
	}
	...
	if (storeKey === "table-storage") {
		...
		const columnWidthsCount = Object.keys(state.columnWidths || {}).filter(
			key => key.startsWith(targetKey.replace(/^(.+?)-(.+)$/, "$1-$2-"))
		).length;
		const rowHeightsCount = Object.keys(state.rowHeights || {}).filter(key =>
			key.startsWith(targetKey.replace(/^(.+?)-(.+)$/, "$1-$2-"))
		).length;

		const parts: string[] = [];
		if (columnWidthsCount > 0) parts.push(`${columnWidthsCount} col widths`);
		if (rowHeightsCount > 0) parts.push(`${rowHeightsCount} row heights`);
		return parts.join(", ") || "Layout data";
	}
	return null;
}
