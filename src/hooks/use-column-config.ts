...
import { mergeFilters, mergeTransformations } from "~/lib/column-config-utils";
import { api } from "~/trpc/react";
import type { FilterConfig, FilterRecord } from "~/types/filters";
import type {
...
export function useColumnConfig({
	...
	const hasTransformationOverride = (columnName: string): boolean => {
		return tableTransformations.some(t => t.columnName === columnName);
	};

	const hasFilterOverride = (columnName: string): boolean => {
		return tableFilters.some(f => f.columnName === columnName);
	};
